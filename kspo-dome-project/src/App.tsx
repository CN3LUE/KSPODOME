...
function MiniGameRun({ isOpen, onClose, myCharacter, onAddJumps, onUpdateBestScore }) {
  ...
  useEffect(() => {
    ...
    const loop = (timestamp = performance.now()) => {
      ...
      ctx.shadowColor = "rgba(0,0,0,0)";
      ctx.shadowBlur = 0;
      // 장애물 50개마다 다음 존으로 이동하고 ZONE 4(RACER)에서 영구 고정됩니다.
      const stageNumber = Math.min(stageThemes.length - 1, Math.floor(clearedObstacles / 50));
      const theme = stageThemes[stageNumber];
      ctx.fillStyle = stageGradients[stageNumber]; ctx.fillRect(0, 0, GAME_WIDTH, GAME_HEIGHT);
      ...
      // 일반 문자열 결합을 사용해 배포 환경에서 백틱이 손상되는 문제를 방지합니다.
